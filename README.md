...
Benefícios esperados (3 bullets)
...
Dentro do escopo (IN): 5–8 itens (funcionalidades)
1 - Cadastro de livros
2 - Cadastro de usuário
3 - Empréstimo de livros 
4- Catálogo de livros
5 - Devolução de livros
6 - Cadastro de administradores
7 - Sistema de bloqueio temporário de empréstimo por atraso de livros

 Fora do escopo (OUT): 3–6 itens (o que não será feito agora)
1 - Envio de emails
2 - Vendas de livros
3 - Sistema de renovação de livro
4 - Reservas de livros

7. Requisitos Funcionais (RF)
Crie de 6 a 10 RFs, numerados e objetivos.
Exemplo de formato:
1 - Cadastrar clientes com campos obrigatórios (Id, nome, telefone, matricula, email, logradouro)
2 - Cadastro de livros
3 - Empréstimo de livros
4 - Devolução de livros
5 -Sistema de multas monetária  por atraso na devolução
6 - Sistema de bloqueio temporário de empréstimo por atraso de livros

8. Regras de Negócio (RN)
Crie 5 a 10 regras, numeradas, claras e verificáveis.
 Exemplos:
1 - Não pode ter empréstimo da mesma cópia do livro para pessoas distintas
2 - A pessoa não pode pegar mais de uma cópia do mesmo livro
3 - Limitar empréstimo de 5 livros por pessoa
4 - Realizar empréstimos apenas para pessoas cadastradas
5 - A mesma pessoa não pode ter mais de um cadastro
6 - Limitar o tempo do empréstimo do livro a 10 dias


9. Requisitos Não Funcionais (RNF) e restrições técnicas
Inclua de 5 a 8 itens:
...
Liste de 4 a 8 pontos, misturando:
...
Liste de 6–12 entidades prováveis (ainda sem diagrama formal), com 3–6 atributos cada.
Ex.: Cliente (id, nome, telefone), Agendamento (dataHora, status), Serviço (nome, preço).

1 - Livro(id, título, autor ,gênero, quantidade)	

2 - Empréstimo(id, id_livro, id_cliente, id_funcionario, data_retirada, data_devolucao_esperada)

3 - Cliente(Id, nome, telefone, matricula, email, logradouro)

4 - Funcionário(Id, nome, telefone, email, senha)

5 - Devolução(id, id_livro, id_cliente, id_funcionario, data_devolucao)

6 - Multa(id, id_emprestimo, valor)

12. Casos de uso (lista + breve descrição)
Liste 5–8 casos de uso, cada um com:
Nome: 
...
Empréstimo atrasado - O Leandro queria realizar a devolução de um livro, porém na devolução o livro estava atrasado e acabou gerando uma multa da qual o Leandro teve que pagar.
